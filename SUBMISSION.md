...
| `/api/verify` | **Streaming citation verifier** | An extractor pulls every legal citation out of the final draft (statute, regulation, schedule paragraph, case decision, agency guidance). The route opens an SSE stream and the UI populates rows in real time as the verifier agent independently checks each citation against a primary source via `web_search`. Per-citation status: `verified`, `mismatch`, `not_found`, `ambiguous`, or `skipped`. Concurrency 4. The UI shows a green/red badge per citation; the PDF packet embeds the same audit table. On the UK DWP demo, 5/5 citations pass with primary-source URLs (legislation.gov.uk, gov.uk, england.shelter.org.uk). |
| `/api/fix-citation` | **Self-correcting reviser** | When the verifier returns a `mismatch` or `not_found`, the user can click "Auto-fix this citation" on that row. A focused fixer agent rewrites only the affected sentence — citing the primary-source URL the verifier already returned, in the same paragraph, in the same legal register, in the same language. Hard rule against introducing any claim the verifier didn't establish. The corrupted-demo flow (reg 19 → reg 18 swap) demonstrates the system catching its own fabrication and self-correcting in ~70 seconds against the actual legislation.gov.uk page. |
...
The killer feature is the **input-grounded → output-verified → self-correcting** spine. The drafter cites only what `web_search` confirmed during generation. The verifier then independently re-checks every citation in the finished letter against primary sources via streaming SSE — the user watches the audit table populate row by row. When the verifier flags a `mismatch` or `not_found`, a single click on that row triggers the fixer agent, which rewrites only the affected sentence using the verifier's primary-source URL. **The system catches its own fabrication and corrects itself, on screen, against the actual statute, in ~70 seconds.**

`/api/harden` is the fourth leg: a counterparty agent reads the draft adversarially as the actual reviewing officer, finds 3–7 specific weaknesses (with the regulation each one will be rejected under), and dispatches researcher sub-agents in parallel to gather real public evidence. A reviser rewrites the draft using that evidence; the verifier can then be re-run on the hardened version.
