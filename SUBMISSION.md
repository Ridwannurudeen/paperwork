...
The killer feature is `/api/harden`. After the drafter writes a response, a second agent reads it adversarially as the actual reviewing officer, finds 3–7 specific weaknesses (with the regulation each one will be rejected under), and dispatches researcher sub-agents in parallel to gather real public evidence. A third agent rewrites the draft using that evidence.
